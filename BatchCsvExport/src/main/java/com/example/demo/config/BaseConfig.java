...
public abstract class BaseConfig {
	...
	@Autowired
	protected FlatFileHeaderCallback csvFileHeaderCallback;
	
	@Autowired
	protected FlatFileFooterCallback csvFileFooterCallback;
	...
	public FlatFileItemWriter<Employee> csvWriter(){
		...
		return new FlatFileItemWriterBuilder<Employee>()
				...
				.resource(outputResource)	//ファイルの出力先
				.append(false)	//追記設定
				.lineAggregator(aggregator)	//区切り文字
				.headerCallback(csvFileHeaderCallback)	//ヘッダー
				.footerCallback(csvFileFooterCallback)	//フッター
				.encoding(StandardCharsets.UTF_8.name())	//文字コードの設定
				.build();
	}
}
